...
% Code-Styling
\usepackage{listings}
\lstdefinestyle{quartocode}{
    basicstyle=\ttfamily\small\color{rub-blue!90!black},
    backgroundcolor=\color{rub-grey!70!white},
    frame=single,
    framerule=0pt,
    columns=flexible,
    keepspaces=true,
...
\begin{frame}{Getting to know Quarto (in VSCode)}
    \begin{columns}[c]
        \begin{column}{0.6\textwidth}
            \begin{itemize}
                \item You should now have 4 files: \texttt{\_quarto.yml}, \texttt{about.qmd}, \texttt{index.qmd}, \texttt{styles.css}
                \item These files form the foundation of a Quarto website. You can already render and preview the site (via the terminal with ``quarto render'' or the render button in the top right of any .qmd file).
                \item The \texttt{\_quarto.yml} is the configuration file for the website; here you can adjust the core design and global settings.
                \item Each .qmd file has a YAML header (marked by \texttt{---} at the top and bottom). Here you can override settings for that specific page.
            \end{itemize}
        \end{column}
        \begin{column}{0.4\textwidth}
...
\begin{frame}[fragile]{Quarto-Syntax}
    \begin{itemize}
        \item \lstinline|#, ##, ###| creates sections, subsections, subsubsections, etc.
        \item \lstinline|:::{} \dots :::| creates callout blocks or tabsets, depending on the content inside \lstinline|{}|.
        \item \lstinline|<details> </details>| creates a foldable HTML block. You can add a title using \lstinline|<summary> </summary>| inside.
        \item \lstinline|```{} ```| creates a code block. The content of \lstinline|{}| defines the language.
    \end{itemize}
\end{frame}

\section{Pyodide-Python}

\begin{frame}[fragile]{Pyodide-Python}
    \begin{itemize}
        ...
        \item The extension can be installed via the terminal with \texttt{quarto add coatless-quarto/pyodide}.
...
\begin{frame}[fragile]{Pyodide-Python}
    \begin{columns}[c]
        \begin{column}{0.6\textwidth}
            \begin{itemize}
                \item To use the extension in your .qmd files, you have to add \texttt{filters: - pyodide} to the YAML header.
                \item Afterward, you can use \lstinline|```{pyodide-python} \dots ```| to embed Python code blocks into the page.
                \item For example: \lstinline|```{pyodide-python} print("Hello World") ```| will create a code block containing \texttt{print("Hello World")}, which can be edited and executed on the website.
            \end{itemize}
        \end{column}
...
\begin{frame}[fragile]{Pyodide-Python in Action}
    \centering
...
\begin{frame}{Multilanguage Website Projects}
    \begin{itemize}
        ...
        \item There is also the small problem of ``quarto preview'' not supporting the dropdown menu, but you can host the website locally and then everything works fine, the local hosting and previewing in the browser is also included in the python script.
    \end{itemize}
\end{frame}

...
def filter_hits_by_unit(hits, unit: str | None, top_k: int):
    """
    Filter hits to only include chunks from units <= specified unit.
    Chunks with unit=None are always included.
    Returns top_k results or empty list if none match.
    """
    if not unit:
        return hits[:top_k]
    ...
    filtered = [h for h in hits if h.unit is None or int(h.unit) <= max_unit]
...
def render_sources(hits):
    ...
    for i, h in enumerate(hits, start=1):
        ...
        # Build URL for PDFs with page anchor
        url = None
        if h.file_type == "pdf" and h.page is not None:
            url = f"/docs/{src}#page={h.page}"
...
@app.get("/")
def index():
    return send_from_directory("ops/www", "index.html")
...
@app.get("/config")
def get_config():
    """Serve configuration including available units."""
    config_path = Path(Config.VECTOR_DB_PATH) / "config.json"
    if config_path.exists():
        return send_from_directory(Config.VECTOR_DB_PATH, "config.json")
...
def ask():
    ...
    # 1) Retrieve from notes with unit filtering
    unit = (data.get("unit") or "").strip()
    q_emb = embed_texts([q])[0]
    top_k = int(data.get("top_k", RAG.TOP_K_DEFAULT))

    # Overfetch 3x and filter by unit
    hits_raw = retriever.search(q_emb, top_k=top_k * 3, log_hits=True)
    hits = filter_hits_by_unit(hits_raw, unit, top_k)

    if not hits:
        return jsonify(
            error=f"No relevant content found in units up to {unit}",
            answer_notes="No relevant course materials found for the selected unit range.",
            answer_extra=None,
            coverage="none",
            sources=[],
        ), 200
    ...
    app.logger.info(
        "SAN notes len=%d tail=%r", len(notes_answer or ""), (notes_answer or "")[-120:]
    )
...
def serve_document(filename):
    """
    Serve documents (PDFs only) from the notes repository.
    Security: Only serves .pdf files to prevent directory traversal.
    """
    from pathlib import Path
    import os

    # Check if NOTES_REPO_DIR is configured
    if not Config.NOTES_REPO_DIR:
        return jsonify(error="Document repository not configured"), 500

    # Security check: only serve PDFs
    if not filename.lower().endswith(".pdf"):
        return jsonify(error="Only PDF files are supported"), 400

    # Resolve the file path within the repo directory
    repo_dir = Path(Config.NOTES_REPO_DIR)
    ...
    try:
        ...
        if not str(file_path).startswith(str(repo_dir_resolved)):
    ...
    return send_from_directory(repo_dir_resolved, filename)
